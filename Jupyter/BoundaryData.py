...
class BoundaryCondition(object):
    def __init__(self, size):
        self.alpha = np.empty(size, dtype = complex)
        self.beta  = np.empty(size, dtype = complex)
        self.f     = np.empty(size, dtype = complex)

class BoundaryIncidence(object):
    def __init__(self, size):
        self.phi = np.empty(size, dtype = complex);
        self.v   = np.empty(size, dtype = complex);
